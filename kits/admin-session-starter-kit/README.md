...
|---|---|---|
| **1** | *Monthly cost, broken down for chargebacks (DE / DS / research), emailed as a CSV* | `sql/cost_by_category.sql` + Genie One prompts 1–3 + the scheduled job |
| **2** | *Alert me when a category is within 10% of its budget* | `sql/budget_threshold_alert.sql` + prompts 4–5 + the alert task in the job |
