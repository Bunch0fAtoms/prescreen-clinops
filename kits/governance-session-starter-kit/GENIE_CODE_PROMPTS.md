...
**Fred Hutch onsite · Governance session · Genie Code over the cloned OMOP tables**

The build is **free-form**. The foundation is already up, the 6 OMOP tables are deep-cloned into your
own governance schema (`clinops_gov`) and `_config` holds the PHI map + the governed-tag
vocabulary. From here **you design the governance solution your data office actually needs.** These are
**starter prompts** to get Genie Code moving. Change them, combine them, reorder them, or ignore them
and ask your own. The goal is a governed warehouse *your team* designed, not a filled-in worksheet.
...
> **🔒 The cardinal rule, say it to Genie Code:** bind masks and row filters **only** to your
> `clinops_gov` schema, **never** the shared source schema. A policy changes what *everyone*
> sees; binding it to a schema the DE/ML tracks read would break their build.
...
> **"Profile the 6 OMOP tables in my `clinops_gov` schema: row counts and columns. Then tell me which columns are patient identifiers vs. clinical facts, and propose how to classify each using only the governed tag values in `_config`'s `PHI_COLUMNS` map. Don't apply anything yet, show me the plan."**

*Good looks like:* a column inventory + a proposed `phi` / `data_sensitivity` tag per column, using the
...
> **"Apply the classification: tag each identifier column with `phi` + `data_sensitivity` and each clinical column with `data_sensitivity` only, using the exact governed values from `PHI_COLUMNS`. Then run a coverage check that lists any PHI column still missing a tag."**
...
> **"Create a `consent_row_filter(person_id)` that returns TRUE for the data office (`NOT is_account_group_member(...)`) and only consented rows for the OCDO group, then `SET ROW FILTER ... ON (person_id)` across the patient tables in my governance schema."**
...
*Good looks like:* a crisp owner-vs-researcher contrast. This is the SOC2-reviewer moment, the policy
holds *in the data layer* no matter who queries (notebook, SQL, Genie, BI).
...
*Good looks like:* a by-value hit list + a tag-based structural inventory, "here is everywhere this
patient identifier lives," which is exactly the question a breach response or a data-subject request asks.
...
You have a governed warehouse, extend it however your data office would:

- *"Define the mask once by tag with a tag-based ABAC `CREATE POLICY` so it applies everywhere `phi` appears."* (Preview-gated on this metastore. Attempt it, treat a gate as a documented ask, not a failure.)
