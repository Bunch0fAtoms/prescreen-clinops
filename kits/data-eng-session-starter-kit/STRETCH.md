...
Finished the core build (notebooks 01 to 05, all the `# TODO (you build this)` markers)? Pick an
...
- Make the reconciliation a **job task that fails on FAIL**, wired after an ingest task so the pipeline
...
- Rewrite `assert_ingest_allowed` to read `information_schema` tag assignments and block any source
  tagged `restricted`, with no separate config table at all.
