...
# MAGIC `reason`, plus the biomarker fields. (These run against the table you built in nb 06.)
...
# MAGIC
# MAGIC Run the SQL you *expect* Genie to generate so you can confirm its number is right. `gold_trial_prescreen`
# MAGIC is LONG, so you filter by `trial_id` and `eligible`.

# COMMAND ----------

# DBTITLE 1,TODO, verify: how many patients are eligible for Trial A?
# MAGIC %sql
# MAGIC -- TODO (you build this): COUNT rows in gold_trial_prescreen where trial_id = 'A' AND eligible = TRUE.
...
# MAGIC %sql
# MAGIC -- TODO (you build this): COUNT where trial_id = 'A' AND eligible = TRUE AND biomarker_source = 'nlp'.
