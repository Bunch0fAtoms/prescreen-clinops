...
# MAGIC
# MAGIC 1. Run the next cell to write `silver_layer.sql` to your workspace (after you finish its TODO).
...
--   patient with three columns: her2_status, er_status, pr_status.
-- WHY: every downstream step (gold, eval, Genie) keys on one biomarker row per
--   patient. The measurement table is long (one row per marker), so you must pivot.
-- PATTERN: MAX(CASE WHEN measurement_source_value = '<marker>' THEN value_source_value END)
--   for each of 'HER2/neu', 'Estrogen receptor', 'Progesterone receptor', GROUP BY person_id.
-- Source table in pipeline form: ${source_catalog}.${source_schema}.measurement
...
SELECT 1;  -- <- replace this whole SELECT with your pivot
...
print("   ⚠ Finish the silver_biomarker_profile TODO before pointing a pipeline at it.")
...
# DBTITLE 1,Verify the silver layer (runs once your pivot exists)
# MAGIC %sql
