...
**Goal: understand the data before you build on it.** Spend your first block interrogating the
foundation so the build that follows is grounded in what the data actually says, not in
assumptions. It goes quickly, because the data starts answering your questions right away.
...
**Discovery also surfaces what to protect.** As you ask real questions of the data, watch which
columns hold patient identifiers and which questions imply one role should see more than another.
Those answers are the starting point for a governance control set (tags, column masks, row
filters), grounded in how the data is actually used. The `person.is_high_profile` flag is included
as a ready row-filter target for that kind of demo.
...
They are read-only. Nothing you do in discovery changes them.
...
key every table joins on). Counts you should see: person 300, condition_occurrence 300,
measurement 720, observation 720, drug_exposure 383, note 265.
...
gap is exactly what the ML build closes with language models.
...
*What you learn:* a first draft of a governance control set. The identifiers you list here are the
columns you would tag, mask, and row-filter, so the controls you apply reflect what the data
actually holds.
...
Move to `../kits/ml-session-starter-kit/`: build the features, recover the notes-only patients
with language models, and produce the trial pre-screen and a Genie space clinical researchers can
question.

The discovery Genie space you build here is worth keeping. It becomes the front door a researcher
uses to ask about the data.
