...
Build this space during discovery. It has a second job too: the questions you ask here point at
what to protect. A question that pulls up patient identifiers is a signal for a mask; a question
one role should not be able to answer is a signal for a row filter. So the space is both the front
door for researchers and the evidence base for governance.
...
- "Which columns here could identify a specific patient?" (a governance-lens question that helps you
  see what to protect)

## A note on trust

The data here is synthetic and Unity Catalog scoped, so the space is safe to share and demo. When
your team points the same pattern at real curated OMOP data, the governance controls you apply
(column masks, row filters, lineage, audit) carry straight over, so the space respects who is
allowed to see what.
